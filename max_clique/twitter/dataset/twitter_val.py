...
class TWITTER_val(InMemoryDataset):
    def __init__(self, config:dict):
        self.config = config
        self.data_path = Path(config['data_dir'])
        super(TWITTER_val, self).__init__(root=self.data_path)
    ...
    def process(self):
        ...
        train_sample_end = int(0.8*total_sample)
        testset = dataset[train_sample:train_sample_end]
...
if __name__ == '__main__':
    ...
    for cfg in configs.iterdir():
        if str(cfg).startswith("configs/config"):
            ...
            dataset = TWITTER_val(cfg_dict['val'])
