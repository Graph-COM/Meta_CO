...
from cut_utils import total_var
from modules_and_utils import GATAConv
###########
torch.pi = torch.acos(torch.zeros(1)).item() * 2

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

class ErdosLoss_cut(torch.nn.Module):
    def __init__(self):
        super(ErdosLoss_cut,self).__init__()
        #self.penalty = Penalty()
    def forward(self, x, probs, edge_index, batch, device):
...
class cut_MPNN(torch.nn.Module):
    ...
    def forward(self, data, tvol = None):
        ...
        #import pdb; pdb.set_trace()
        #print(recvol_hard.mean()/totalvol.mean())
        #print(recvol_hard.mean())
        #print(totalvol.mean())
        assert recvol_hard.mean()/totalvol.mean() <=1.01, "Something went wrong! Receptive field is larger than total volume."
...
class ErdosLoss_clique_badloss(torch.nn.Module):
    def __init__(self):
        super(ErdosLoss_clique_badloss,self).__init__()
        #self.penalty = Penalty()
    def forward(self, probs, edge_index, batch, penalty_coefficient,device):
        #calculating the terms for the expected distance between clique and graph
        probs = torch.sin(8.5 * torch.pi * probs)
        num_graphs = batch.max().item() + 1
        no_loop_index,_ = remove_self_loops(edge_index)  
        no_loop_row, no_loop_col = no_loop_index
        pairwise_prodsums = torch.zeros(num_graphs, device = device)
...
class clique_MPNN_gumbel(torch.nn.Module):
    def __init__(self, dataset, num_layers, hidden1, hidden2, deltas):
        super(clique_MPNN_gumbel, self).__init__()
        self.hidden1 = hidden1
        self.hidden2 = hidden2
        self.momentum = 0.1
        self.convs = torch.nn.ModuleList()
