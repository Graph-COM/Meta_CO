...
def optimize(vertices, edges):
    ...
    vertexVars = {}
    for v in vertices:
        vertexVars['x_'+str(v)] = m.addVar(vtype=GRB.BINARY, name="x_"+str(v))
    count_edges = 0
    for edge in edges:
        m.addConstr(vertexVars['x_'+str(edge[0])] + vertexVars['x_'+str(edge[1])] >= 1, name= 'c_'+str(count_edges))
        count_edges = count_edges + 1
    m.setObjective(sum([vertexVars['x_'+str(node)] for node in vertices]), GRB.MINIMIZE)
    #m.write('test.lp')
    m.optimize()
    coversize = m.objVal
    time_used = m.Runtime
    return coversize, time_used
...
class RB200_val(InMemoryDataset):
    def __init__(self, config:dict):
        self.config = config
        self.data_path = Path(config['data_dir'])
        super(RB200_val, self).__init__(root=self.data_path)
    ...
    def process(self):
        ...
        for task_index in tqdm(range(50)):
            '''
            ...
            for data in tmp_data_loader:
                ...
                final_data = Data(x = new_data.x, edge_index = new_data.edge_index, min_cover = n*k-n)
...
if __name__ == '__main__':
    ...
    for cfg in configs.iterdir():
        if str(cfg).startswith("configs/config"):
            ...
            dataset = RB200_val(cfg_dict['val'])
