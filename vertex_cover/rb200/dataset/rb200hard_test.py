...
class RB200hard_test(InMemoryDataset):
    def __init__(self, config:dict):
        self.config = config
        self.data_path = Path(config['data_dir'])
        super(RB200hard_test, self).__init__(root=self.data_path)
    ...
    def process(self):
        ...
        for task_index in tqdm(range(100)):
            ...
            for data in tmp_data_loader:
                ...
                final_data = Data(x = new_data.x, edge_index = new_data.edge_index, min_cover = cover)
...
if __name__ == '__main__':
    ...
    for cfg in configs.iterdir():
        if str(cfg).startswith("configs/config"):
            ...
            dataset = RB200hard_test(cfg_dict['test'])
